...
export default function App() {
  ...
  const analyzeImage = async () => {
    ...
    // 💡 1. 구글 키 직접 입력 로직 제거 및 환경 변수 전용으로 깔끔하게 원복
    let finalApiKey = "";
    ...
    // 💡 2. 도면 분석 프롬프트 및 출력 형식은 '하루 전(치수선 그리기)' 방식으로 완벽 원복
    const payload = {
      contents: [
        {
          parts: [
      ...
      systemInstruction: {
        parts: [
          {
            text: "너는 건축 CAD 도면 분석가야. 치수보조선은 오직 측정 객체인 '굵은 붉은색(마젠타 포함) 실선'에서만 나와야 하며(규칙 1), 방을 나누는 벽체일 때만 예외적으로 '검은색 실선'에서 나온다(규칙 2). 이 2가지 경우를 제외한 어떤 곳(복도, 허공 등)에서도 절대 치수선의 기점(x, y)을 잡지 마라. 이 규칙을 엄격하게 지켜 쪼개진 벽면 선분들의 양 끝점 좌표를 JSON(JavaScript Object Notation)으로 출력해.",
          },
        ],
      },
      generationConfig: {
                    ...
                    items: {
                      ...
                      properties: {
                        ...
                        x1: {
                          type: "NUMBER",
                          description:
                            "측정 객체(굵은 붉은선)의 시작 기점 X 좌표 (반드시 규칙 1, 2 위치)",
                        },
                        y1: {
                          type: "NUMBER",
                          description:
                            "측정 객체(굵은 붉은선)의 시작 기점 Y 좌표 (반드시 규칙 1, 2 위치)",
                        },
                        x2: {
                          type: "NUMBER",
                          description:
                            "측정 객체(굵은 붉은선)의 끝 기점 X 좌표 (반드시 규칙 1, 2 위치)",
                        },
                        y2: {
                          type: "NUMBER",
                          description:
                            "측정 객체(굵은 붉은선)의 끝 기점 Y 좌표 (반드시 규칙 1, 2 위치)",
                        },
                      },
                      required: [
                        "orientation",
  ...
  // 💡 3. Canvas 그리기 로직: 파란색 치수선을 긋는 '하루 전' 방식으로 완벽 복구
  useEffect(() => {
      ...
      <div className="w-full max-w-2xl bg-white rounded-3xl border border-zinc-200/60 shadow-[0_8px_40px_-12px_rgba(0,0,0,0.08)] overflow-hidden flex flex-col min-h-[85vh]">
        <div className="px-8 pt-8 pb-2">
          ...
          <p className="text-zinc-500 text-sm mt-2.5 font-medium leading-relaxed">
            측정 객체(붉은선/마젠타선)와 기점 규칙을 엄격히 통제하여 치수선을
            그립니다.
          </p>
        </div>
